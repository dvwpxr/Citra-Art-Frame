...
func GetArtPrints(w http.ResponseWriter, r *http.Request) {
	rows, err := database.DB.Query("SELECT id, title, artist, category, description, price, stock, image_url, created_at FROM art_prints ORDER BY id DESC")
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Title, &p.Artist, &p.Category, &p.Description, &p.Price, &p.Stock, &p.ImageURL, &p.CreatedAt); err != nil {
...
func GetArtPrint(w http.ResponseWriter, r *http.Request) {
	...
	err = database.DB.QueryRow("SELECT id, title, artist, category, description, price, stock, image_url, created_at FROM art_prints WHERE id = ?", id).Scan(&p.ID, &p.Title, &p.Artist, &p.Category, &p.Description, &p.Price, &p.Stock, &p.ImageURL, &p.CreatedAt)
	if err != nil {
...
func CreateArtPrint(w http.ResponseWriter, r *http.Request) {
	...
	result, err := database.DB.Exec(
		"INSERT INTO art_prints (title, artist, category, description, price, stock, image_url, public_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
		p.Title, p.Artist, p.Category, p.Description, p.Price, p.Stock, p.ImageURL, p.PublicID,
	)
	if err != nil {
...
func UpdateArtPrint(w http.ResponseWriter, r *http.Request) {
	...
	_, err = database.DB.Exec(
		"UPDATE art_prints SET title=?, artist=?, category=?, description=?, price=?, stock=?, image_url=?, public_id=?, updated_at=NOW() WHERE id=?",
		p.Title, p.Artist, p.Category, p.Description, p.Price, p.Stock, p.ImageURL, p.PublicID, id,
	)
	if err != nil {
